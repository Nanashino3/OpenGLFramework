...
namespace tnl {
	std::tuple<std::shared_ptr<char>, uint32_t, uint32_t, uint32_t, uint32_t>
		CreateFontTexture(
			...
			bool is_vertical,
			int32_t thickness,
			const tkl::Vector3& thickness_color,
			const std::string& font_type,
			const tkl::Vector3& top_color,
			const tkl::Vector3& bottom_color) {
		...
		tkl::Vector3 color;
		tkl::Vector3 csub = bottom_color - top_color;
		...
		for (y = iOfs_y; y < (iOfs_y + iBmp_h); y++)
		{
			...
			for (x = iOfs_x; x < iOfs_x + iBmp_w; x++)
			{
				...
				int R = int((color.mX * float(Alpha)));
				int G = int((color.mY * float(Alpha)));
				int B = int((color.mZ * float(Alpha)));
				...
				R = int((thickness_color.mX * float(Alpha)));
				G = int((thickness_color.mY * float(Alpha)));
				B = int((thickness_color.mZ * float(Alpha)));
				Color = (Alpha << 24) | (R << 16) | (G << 8) | (B);
